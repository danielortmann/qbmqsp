...
def xxz_hamiltonian(n: int, J: float = 0.5, Δ: float = 0.75) -> Hamiltonian:
    """Construct a representation of the Heisenberg XXZ Hamiltonian."""
    h = list()
    for p in ['X', 'Y', 'Z']:
        for i in range(n - 1):
...
def xxz_gibbs_state(n_qubits: int, J: float = 0.5, Δ: float = 0.75, β: float = 1.) -> np.ndarray[complex]:
...
def basis_encoding(p: list[float]) -> np.ndarray[float]:
    """Encode discrete probability distribution `p` into a rank-1 density matrix.
...
def gen_boltzmann_dist(n, β=1):
    """Generate an NP-hard Boltzmann distribution of a random classical `n`-qubit Ising model at inverse temperature `β`."""
    seed = 42
...
def gen_discrete_gauss_dist(n):
    """Generate a discrete proximity to the Gaussian distribution of `n` states."""
